...
func save_data():
	var _err = ResourceSaver.save(path_to_res, data, ResourceSaver.FLAG_REPLACE_SUBRESOURCE_PATHS)

func render():
	rendered_codes = []
	for ch in content.get_children():
		ch.queue_free()
	for ch in road.get_children():
		ch.queue_free()

	yield(get_tree(), "idle_frame")
	render_speech(find_speech(0), 0)
...
func render_speech(speech, row):
	...
	for ch in speech.choice_list:
		if has_link(ch):
			...
			render_roadline(new_speech.find_choice_node(ch.code), find_speech_node(ch.link))
...
func create_speech():
	...
	speech.code = get_next_code_index(data.speech_list)
...
func _on_speech_edit(speech_code):
	var speech = find_speech(speech_code)
	if speech != null:
...
func _on_choice_add(speech_code):
	var index = get_list_item_index(speech_code, data.speech_list)
	var choice = choice_res_class.new()
	choice.code = get_next_code_index(data.speech_list[index].choice_list)
	choice.text = "Новая реплика"

	if data.speech_list[index].choice_list == null:
		data.speech_list[index].choice_list = []

	data.speech_list[index].choice_list.append(choice)

	save_data()
	render()

func _on_EditSpeech_apply(speech):
	var index = get_list_item_index(speech.code, data.speech_list)

	if index != null:
		data.speech_list[index] = speech
		save_data()
		render()
...
func _on_EditChoice_apply(speech_code, choice_res):
	var speech_index = get_list_item_index(speech_code, data.speech_list)

	if speech_index == null:
		return

	var choice_index = get_list_item_index(choice_res.code, data.speech_list[speech_index].choice_list)

	if choice_index == null:
		return

	data.speech_list[speech_index].choice_list[choice_index] = choice_res
	save_data()
	render()
...
func _on_choice_edit(speech_code, choice_code):
	...
	var choice_index = get_list_item_index(choice_code, data.speech_list[speech_index].choice_list)

	if choice_index == null:
		return

	edit_choice_hud.open(speech_code, data.speech_list[speech_index].choice_list[choice_index])
...
func _on_choice_update_order(speech_code, choice_order_code):
	...
	for code in choice_order_code:
		for ch_item in data.speech_list[speech_index].choice_list:
			if ch_item.code == code:
				ordered.append(ch_item)
				break

	data.speech_list[speech_index].choice_list = ordered
	save_data()
	render()
...
func _on_choice_link_create(speech_code, choice_code):
	...
	data.speech_list.append(new_speech)
	data.speech_list[speech_index].choice_list[choice_index].link = new_speech.code
...
func _on_speech_linked(speech_code):
	...
	var speech_index = get_list_item_index(waiter.speech, data.speech_list)

	if speech_index == null:
		return

	var choice_index = get_list_item_index(waiter.choice, data.speech_list[speech_index].choice_list)

	if choice_index == null:
		return

	data.speech_list[speech_index].choice_list[choice_index].link = speech_code
